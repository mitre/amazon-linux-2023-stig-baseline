control 'SV-274045' do
  ...
  desc 'check', %q(Verify Amazon Linux 2023 is configured so that the SSH daemon does not allow Kerberos authentication with the following command:
...
If the value is returned as "yes", the returned line is commented out, no output is returned, and the use of Kerberos authentication has not been documented with the information system security officer (ISSO), this is a finding.)
  desc 'fix', 'Configure Amazon Linux 2023 so that the SSH daemon does not allow Kerberos authentication.
...
$ sudo systemctl restart sshd.service'
  ...
  tag gid: 'V-274045'
  tag rid: 'SV-274045r1120123_rule'
  tag stig_id: 'AZLX-23-001220'
  tag fix_id: 'F-78041r1120122_fix'
