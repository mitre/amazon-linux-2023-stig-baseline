control 'SV-274141' do
  title 'Amazon Linux 2023 must limit the number of concurrent sessions to ten for all accounts and/or account types.'
...
This requirement addresses concurrent sessions for information system accounts and does not address concurrent sessions by single users via multiple system accounts. The maximum number of concurrent sessions should be defined based on mission needs and the operational environment for each system.'
  desc 'check', %q(Verify Amazon Linux 2023 limits the number of concurrent sessions to "10" for all accounts and/or account types with the following command:
...
If the "maxlogins" item is missing, commented out, or the value is set greater than "10" and is not documented with the information system security officer (ISSO) as an operational requirement for all domains that have the "maxlogins" item assigned, this is a finding.)
  desc 'fix', 'Configure Amazon Linux 2023 to limit the number of concurrent sessions to "10" for all accounts and/or account types.
...
* hard maxlogins 10'
  ...
  tag gid: 'V-274141'
  tag rid: 'SV-274141r1120411_rule'
  tag stig_id: 'AZLX-23-002395'
  tag fix_id: 'F-78137r1120410_fix'
  tag cci: ['CCI-000054']
  tag nist: ['AC-10']
  tag 'host'
