control 'SV-274168' do
  title 'Amazon Linux 2023 must allocate an audit_backlog_limit of sufficient size to capture processes that start prior to the audit daemon.'
...
system is susceptible to boot failures and crashes.'
  desc 'check', %q(Verify Amazon Linux 2023 allocates a sufficient audit_backlog_limit to capture processes that start prior to the audit daemon with the following command:
...
If the command returns any outputs, and audit_backlog_limit is less than "8192", this is a finding.)
  desc 'fix', 'Configure Amazon Linux 2023 to allocate sufficient audit_backlog_limit to capture processes that start prior to the audit daemon with the following command:
...
$ sudo grubby --update-kernel=ALL --args=audit_backlog_limit=8192'
  ...
  tag gid: 'V-274168'
  tag rid: 'SV-274168r1120492_rule'
  tag stig_id: 'AZLX-23-002520'
  tag fix_id: 'F-78164r1120491_fix'
