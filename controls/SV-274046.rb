control 'SV-274046' do
  title 'Amazon Linux 2023 must force a frequent session key renegotiation for SSH connections to the server.'
...
compromised.'
  desc 'check', %q(Verify Amazon Linux 2023 is configured so that the SSH forces frequent session key renegotiation with the following command:
...
If "RekeyLimit" does not have a maximum data amount and maximum time defined, is missing, or is commented out, this is a finding.)
  desc 'fix', 'Configure Amazon Linux 2023 to force a frequent session key renegotiation for SSH connections to the server by adding or modifying the following line in the "/etc/ssh/sshd_config" file or in a file in "/etc/ssh/sshd_config.d":
...
$ sudo systemctl restart sshd.service'
  ...
  tag gid: 'V-274046'
  tag rid: 'SV-274046r1120126_rule'
  tag stig_id: 'AZLX-23-001225'
  tag fix_id: 'F-78042r1120125_fix'
