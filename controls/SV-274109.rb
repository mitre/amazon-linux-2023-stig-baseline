control 'SV-274109' do
  title 'Amazon Linux 2023 audit log directory must be owned by root to prevent unauthorized read access.'
  desc 'Unauthorized disclosure of audit records can reveal system and configuration data to attackers, thus compromising its confidentiality.

Audit information includes all information (e.g., audit records, audit settings, audit reports) needed to successfully audit operating system activity.'
  desc 'check', 'Verify Amazon Linux 2023 audit logs directory is owned by "root". 
...
If the audit log directory is not owned by "root", this is a finding.'
  desc 'fix', 'Configure Amazon Linux 2023 so that the audit logs directory is protected from unauthorized read access by setting the correct owner as "root" with the following command:
...
$ sudo chown root /var/log/audit'
  ...
  tag gid: 'V-274109'
  tag rid: 'SV-274109r1120315_rule'
  tag stig_id: 'AZLX-23-002230'
  tag fix_id: 'F-78105r1120314_fix'
