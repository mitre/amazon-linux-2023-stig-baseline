control 'SV-274014' do
  title 'Amazon Linux 2023 must require reauthentication when using the "sudo" command.'
  desc 'Without reauthentication, users may access resources or perform tasks for which they do not have authorization.'
  desc 'check', %q(Verify Amazon Linux 2023 requires reauthentication when using the "sudo" command to elevate privileges with the following command:
...
If "timestamp_timeout" is set to a negative number, is commented out, or no results are returned, this is a finding.)
  desc 'fix', 'Configure Amazon Linux 2023 to reauthenticate "sudo" commands after the specified timeout:
...
Defaults timestamp_timeout=0'
  ...
  tag gid: 'V-274014'
  tag rid: 'SV-274014r1120030_rule'
  tag stig_id: 'AZLX-23-001010'
  tag fix_id: 'F-78010r1120029_fix'
