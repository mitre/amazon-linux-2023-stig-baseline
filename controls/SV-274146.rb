...
$ sudo chage -E $(date -d +3days +%Y-%m-%d) system_account_name'
  ...
  # NOTE: SV-274150 is extremely similar; both consume the same inputs.
  tmp_users = input('temporary_accounts')
  tmp_max_days = input('temporary_account_max_days')

  # TODO(reviewer): STIG check text says "chage -l ... | grep 'account expires'".
  # The user() resource's `warndays` is days-before-password-expiry, NOT account
  # expiration date. Likely needs etc_shadow.where { user == u }.expiry_dates.
  # Filed for follow-up review pass on the temp-account cluster.
