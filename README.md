...
This InSpec Profile was created to facilitate testing and auditing of `Amazon Linux 2023`
...
The Amazon Linux 2023 STIG Profile uses the [InSpec](https://github.com/inspec/inspec)
...
This profile is in active development.

The 187 controls were derived from the [RHEL 9 V2R7 InSpec baseline](https://github.com/mitre/redhat-enterprise-linux-9-stig-baseline) using `saf generate delta -M` with a 3-tier requirement-first matcher (SRG-OS blocking + CCI Jaccard tiebreak + Fuse title fallback). 186 of 187 controls (99.5%) mapped structurally to a RHEL 9 ancestor; 1 (`SV-274030`) is a genuinely novel AL2023 requirement with no ancestor.

Per-control review and adaptation for Amazon Linux 2023-specific guidance is in progress. Inherited control bodies are correct in shape (right SRG, CCI, title, metadata) but the test logic still references RHEL 9 commands, paths, and conventions in places. Reviewer attention is currently focused on:

- 11 controls flagged as `potentialMismatch` (Tier 2 primary with CCI Jaccard < 0.5 OR Tier 3 primary with Fuse confidence < 0.9) — see `delta.json` provenance
- 12 controls with explicit `red hat` / RHEL string-match logic in `describe` blocks
- FIPS / SELinux / RHN clusters flagged in the structural diff

Track open review work via the project board.
...
Requirements associated with the Amazon Linux 2023 STIG are derived from the
...
The Amazon Linux 2023 STIG profile checks were developed to provide technical implementation
...
```yaml
  ...
  # used by the Amazon Linux 2023 STIG profile.
...
```bash
# Using `ssh` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/amazon-linux-2023-stig-baseline/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>

# Using `winrm` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/amazon-linux-2023-stig-baseline/archive/master.tar.gz --target winrm://<hostip> --user '<admin-account>' --password=<password> --input-file=<path_to_your_inputs_file/name_of_your_inputs_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/amazon-linux-2023-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive amazon-linux-2023-stig-baseline
...
```bash
cd amazon-linux-2023-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive amazon-linux-2023-stig-baseline --overwrite
